...
client.on("guildMemberAdd", async (member) => {
    ...
    const hoşgeldin_kanalı = client.channels.cache.get("928289333335691374")//mesajın gideceği kanal
    if (member.user.bot) return; // bota mesaj atmama kısmı
    await member.setNickname(`İstiklal Kayıtsız`)// oto isim kısmı
    const jahkyembed = new Discord.MessageEmbed()
        .setTitle(`<a:giris:899297926793281557> | İstiklal'a Hoşgeldin`)
        .setDescription(`
...
message.channel.send('IP adresimiz; 185.193.165.49 \nTıkla Bağlan; steam://connect/185.193.165.49:27015:27015');
}
})
//
client.on("message", async(message) => {
...
client.channels.cache.get("928289333335691374").send(`<@${message.author.id}> Seviye atladı **${lvl || "0"}**`)
...
client.on("ready", () => {
  client.channels.cache.get("1085927875452092617").join();
});
