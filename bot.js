...
const steam = new SteamAPI('1ECE031157C5ED344EB6A35C344484C0');
...
message.channel.send('https://discord.gg/cTEd4PExzu')
}
})
//
client.on("message", async(message) => {
if(message.content === "!ip") {
message.channel.send('IP adresimiz; 195.193.165.47 \nTıkla Bağlan; steam://connect/195.193.165.47:27015');
}
})
//
client.on("message", async(message) => {
if(message.content === "!grup") {
message.channel.send('__Steam Topluluğumuz__\nhttps://steamcommunity.com/groups/yitikjb')
}
})
//
client.on("message", async(message) => {
  if(message.content === "!aktif") {
  message.channel.send(' @here İstiklal Jailbreak Sunucumuz Aktif | İP : steam://connect/185.193.165.47:27015 | Lütfen Müsait Olan Üyelerimiz Ve Yetkililerimiz Sunucuya Girebilirmi.')
  }
  })
  //
  client.on('message', msg => {
    if (msg.content.toLocaleLowerCase() === 'sa') {
      msg.reply('Aleyküm Selam GARDAŞ');
    }
  });
//
client.on('message', msg => {
  if (msg.content.toLocaleLowerCase() === 'oç') {
    msg.delete()
    msg.channel.send('Lütfen Bu Kelimeyi kullanma');
  }
});
////////////////////////////////////////////
